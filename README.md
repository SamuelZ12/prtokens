...
If this prints an absolute path, use that path as the target hooks directory and place or merge the `pre-push` hook there. Do not overwrite existing hooks. If a shell `pre-push` file already exists, prefer appending the managed block below to the end of that file. `prtokens init` fails safely instead of appending when the existing hook's last meaningful line is terminal `exit` or `exec`, because the managed block would be unreachable. When `prtokens init` appends to an existing foreign shell hook, the appended block does not replay repository-local hooks because the original `pre-push` stdin may already have been consumed. Fresh prtokens-managed global hooks still capture stdin and replay repository-local hooks. Keep the manual block below after any logic that needs original `pre-push` stdin, because it captures stdin with `cat > "$stdin_file"`. If you manually place it before trailing commands, those commands must not depend on stdin unless you adapt the hook to replay `"$stdin_file"` to them.
