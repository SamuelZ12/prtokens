...
const FOOTER = '*Generated by [prtokens](https://github.com/SamuelZ12/prtokens/)*';
const LEGACY_OPENCODE_SESSION_NOTE = '_OpenCode child sessions are grouped by parent session._';
...
function parseAuthorSections(body: string): AuthorSection[] {
  ...
  for (const [index, match] of markers.entries()) {
    try {
      ...
      sections.push({
        summary: parsed,
        section: removeLegacyOpenCodeSessionNote(body.slice(match.index, sectionEnd).trimEnd()),
      });
    } catch {
...
function removeLegacyOpenCodeSessionNote(section: string): string {
  return section
    .split('\n')
    .filter((line) => line.trimEnd() !== LEGACY_OPENCODE_SESSION_NOTE)
    .join('\n');
}
