...
const FOOTER = '*uses agent-reported costs when available; otherwise estimates from token pricing \u00b7 generated by prtokens*';
...
export function renderPrComment(input: RenderPrCommentInput): string {
  ...
  const authors = replaceCurrentAuthor(existingAuthors, input.currentAuthor, currentSummary);
  ...
  return [
    ...
    '',
    ...authors.flatMap((author) => [author.section, '']),
    FOOTER,
  ].join('\n');
}
...
function replaceCurrentAuthor(
  ...
  let replaced = false;
  const currentSection = renderCurrentAuthorSection(currentAuthor, currentSummary).join('\n').trimEnd();
