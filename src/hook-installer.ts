...
function renderManagedBlock(prtokensBinPath: string, includeFinalExit: boolean, includeRepoLocalForwarding: boolean): string {
  ...
  lines.push(
    ...
    '    read local_ref local_sha remote_ref remote_sha < "$stdin_file" || true',
    '    rm -f "$stdin_file"',
    '  fi',
    '  zero_sha=0000000000000000000000000000000000000000',
    '  if [ -n "$remote_name" ] && [ -n "$local_sha" ] && [ -n "$remote_ref" ] && [ "$local_sha" != "$zero_sha" ]; then',
    '    attempts=0',
    ...
    '    done',
    '  fi',
    '  prtokens_local_sha="$local_sha" prtokens_remote_sha="$remote_sha" "$prtokens_bin" >/dev/null 2>&1 </dev/null',
    ') >/dev/null 2>&1 &',
  );
